...
(function (angular) {
	 ...
	 *
	 * This directive accepts the next settings parameters:
	 *  -  ifc-availability: The itself directive attribute must have the url to send the request.
	 *  - name: Used to name the parameter to send the value to check, if it doesn't exist, it uses by
	 *  default 'value'.
	 *  - pattern: Regular expression to check the input value, if it matches the regular expression,
	 ...
	 *      the ngPattern directive is execute before it and if it fails then this directive it is not
	 *      executed)
	 *  - additional-data: Object whose attributes will be sent to the server in each request.
	 *  - id: The id attribute is used as prefix for the errors properties names reported, if it
	 *    doesn't exist then 'availability' is used by default.
	 *
	 *  NOTE if the object contains an attribute with the same name that 'name' attribute, then
	 *  it will be overridden by the input value.
	 *
	 * Errors reported, bear in mind that they are prepended by the `id` value as commented above:
	 *   Unaccepted: when the slug doesn't pass the pattern
	 *   Anavailable: when the server reply with not accepted slug response
